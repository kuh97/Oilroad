...
### 5.3.1 용량 한계 ★★ — 오피넷 일일 한도 확인 결과 (1,500회/일)

**한도가 확인되었습니다: 하루 1,500회.** 이는 예상보다 훨씬 낮고, 이 서비스의 **운영 규모를 근본적으로 제약합니다.** 알고리즘 자체를 바꿀 필요는 없지만, "몇 명이 하루에 이 서비스를 쓸 수 있는가"라는 질문에 답이 정해져 버립니다.

**캐시 미스 기준 하루 처리 가능 검색 수 (전 사용자 합산)**

| 시나리오                   | 검색당 오피넷 호출 | 1,500회로 가능한 검색 수 |
| -------------------------- | ------------------ | ------------------------ |
| 확장 없음 (T1+T2 충분)     | 13                 | **약 115건/일**          |
| **확장 발동** (T1+T2 부족) | 39                 | **약 38건/일**           |

**여기서 문제가 됩니다: 확장이 가장 자주 발동하는 세그먼트가 정확히 1순위 타깃(LPG 장거리)입니다.** [`PRODUCT.md`](PRODUCT.md) §1.4·§1.5가 명시한 대로, LPG·저밀도 구간일수록 T1+T2가 부족해 확장이 걸립니다. 즉 **핵심 타깃 사용자일수록 검색 1건이 예산을 3배 더 씁니다.** 하루 38명의 LPG 장거리 검색만으로 전체 예산이 소진될 수 있다는 뜻입니다.
...
| 옵션                      | 상태                 | 내용                                                                             |
| ------------------------- | -------------------- | -------------------------------------------------------------------------------- |
| **A. 베타·소규모로 시작** | **채택 (지금)**      | `OPINET_DAILY_BUDGET=1400`. 예산 소진 시 **신규 검색만 안내로 막는다** (§5.3.2)  |
...
> **B가 "보류"라는 것은 "안 한다"가 아닙니다.** 트래픽이 늘어 A의 상한(하루 약 38~115건)에 근접하면, 유료 API 견적 문의를 **다시 안건으로 올리십시오.** `search_event` 테이블(§7.3)의 일별 검색 건수를 모니터링해 이 시점을 판단합니다.
...
```
OPINET_DAILY_BUDGET = 1400   (확인된 한도 1,500의 ~93% — 안전 여유 100회)
```
...
> ⚠️ **실기기 매트릭스 테스트가 필요합니다(§12 ⑤).** 딥링크는 이 서비스의 최종 전환 지점이라 여기서 막히면 앞의 모든 계산이 무의미해집니다.
...
- 오피넷 일일 예산 카운터도 접두사로 분리하되, **실제 한도(1,500회)는 계정 단위이므로 두 환경의 합이 한도를 넘지 않게** `OPINET_DAILY_BUDGET`을 나눠 설정. 예: dev 100 / prod 1,300. **검증 스크립트(§10 Phase 0·Phase 5)를 prod 예산으로 돌리지 마십시오** — 하루 1,500회를 개발 검증이 잠식하면 운영에 남는 예산이 없습니다
...
| #   | 확인                       | 결과                                                                                                                             |
| --- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| ①   | 오피넷 일일 호출 한도      | **1,500회/일.** 알고리즘은 그대로지만 운영 규모가 크게 제한됨 → §5.3.1 용량 분석. 대응은 "소규모 베타"로 결정됨                  |
