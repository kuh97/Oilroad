...
### 1. 프로젝트 루트에 스크립트 폴더 배치

```
oilroad/              ← Next.js 프로젝트 루트
  scripts/
    _shared.ts
    verify/
      coord.ts
...
> **중요:** 이 스크립트는 Next.js 앱과 별개의 `package.json`을 씁니다.
> 나중에 Next.js 프로젝트를 초기화(Phase 1)하면 `proj4`·`csv-parse`를
> 루트 `package.json`의 devDependencies로 옮기고 `scripts/package.json`을
> 삭제하십시오.
