...
> **서비스 도착점은 소규모 실사용(지인·베타 수십 명)입니다.** 오피넷 하루 1,500회 한도 때문에 규모가 제약되며, 배치 사전수집·유료 API는 의도적으로 채택하지 않았습니다. 재검토 신호는 [`AGENTS.md`](AGENTS.md) §16.4에 있습니다.
...
> `verify:t3-rate`와 `verify:coverage`는 오피넷을 대량 호출합니다. 실행 전 예상 호출 수를 출력하고 확인을 받도록 구현하십시오. **하루 1,500회 예산을 개발 검증이 잠식하면 운영에 남는 게 없습니다.** dev 예산(기본 100회) 안에서 돌리십시오.
...
| `OPINET_DAILY_BUDGET`       | —    | 오피넷 일일 호출 상한 (기본 `1400`). **오피넷 확인 한도가 하루 1,500회**이므로 안전 여유를 두고 낮게 잡습니다. 초과 시 확장 수집을 건너뜀 — 용량 분석은 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) §5.3.1 |
...
| 키     | 발급처                                                                         | 주의                                                                                                                                        |
| ------ | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| 오피넷 | [opinet.co.kr 오픈 API](https://www.opinet.co.kr/user/custapi/openApiIntro.do) | **일일 호출 한도 확인됨: 1,500회.** 상업적 이용은 사전 협의 필요 (052-216-2514)                                                             |
