...
vi.mock("@/services/place-service", () => ({ searchPlaces: (...args: unknown[]) => searchPlacesMock(...args) }));
...
describe("GET /api/places/search", () => {
  ...
  it("fetchPlaces가 실패해도(환경변수 누락 등) 500 HTML이 아니라 구조화된 JSON 에러를 반환한다", async () => {
    searchPlacesMock.mockRejectedValue(new Error("환경변수 KAKAO_REST_API_KEY가 설정되지 않았습니다."));
